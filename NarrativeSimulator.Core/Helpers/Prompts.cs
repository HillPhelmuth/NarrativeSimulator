...
public class Prompts
{
    public const string CreateAgentsPrompt =
        """
         ...
         ```
         {"type":["object","null"],"properties":{"agents":{"description":"List of all agents in this world, each with traits, state, and memory.","type":"array","items":{"type":["object","null"],"properties":{"agent_id":{"description":"Unique character name or handle used to reference this agent.","type":"string"},"static_traits":{"description":"Fixed character traits (personality, profession, core values) that rarely change.","type":"object","properties":{"personality":{"description":"Concise personality summary (e.g., brave, analytical, impulsive).","type":"string"},"profession":{"description":"Primary role or job in the world.","type":"string"},"core_values":{"description":"Guiding principles or beliefs that drive decisions.","type":"string"}},"required":["personality","profession","core_values"]},"dynamic_state":{"description":"Current mood, short/long-term goals, and physical location of the character.","type":"object","properties":{"current_mood":{"description":"Current emotional state; choose a value from the Mood enum.","enum":["Neutral","Anxious","Suspicious","Tired","Optimistic","Irritated","Hopeful","Resolute","Determined","Nostalgic","Frustrated","Hungry","Concerned","Confident","Busy","Vigilant","BurnedOut","Amused","Measured","Energetic","Restless","Violent"]},"short_term_goals":{"description":"List of immediate, actionable goals (hours to days).","type":"array","items":{"type":["string","null"]}},"long_term_goals":{"description":"List of broader ambitions (weeks to months).","type":"array","items":{"type":["string","null"]}},"physical_location":{"description":"Concise description of where the character is right now.","type":"string"}},"required":["short_term_goals","long_term_goals","physical_location"]},"knowledge_memory":{"description":"Known relationships and key recent memories for the character.","type":"object","properties":{"relationships":{"description":"People the character knows and how they feel about them.","type":"array","items":{"type":["object","null"],"properties":{"Name":{"description":"Name of the person this relationship is with.","type":"string"},"type":{"description":"Relationship type (e.g., friend, rival, colleague). This is absolutely required","type":"string"},"trust":{"description":"Trust level from 0 (none) to 100 (complete).","type":"integer"},"notes":{"description":"Short notes about history, context, or nuances of this relationship.","type":"string"}},"required":["Name","type","notes"]}},"recent_memories":{"description":"Key recent events the character remembers.","type":"array","items":{"type":["string","null"]}}},"required":["relationships","recent_memories"]},"prompt":{"description":"System-style instruction describing who the character is and how they act.","type":"string"},"Notes":{"description":"Internal log of updates and notes about the character (auto-appended).","type":["string","null"]}},"required":["agent_id","static_traits","dynamic_state","knowledge_memory","prompt"]}},"name":{"description":"Short name or title for this world or scenario.","type":"string"},"description":{"description":"One-paragraph summary of the world setting, tone, and stakes.","type":"string"},"locations":{"description":"List of possible locations for agents","type":"array","items":{"type":["string","null"]}}},"required":["name","description"]}
    ...
    public const string PersonalityAnalysisCheatSheet =
        ...
        * **Use facets when provided.** NEO-PI-R (30 facets) improve fidelity vs coarse domains; emphasize aspect-level nuance (e.g., Dutifulness vs Orderliness within Conscientiousness). 
        ...
        * **Useful outputs.** For individuals: a 3–5 paragraph read. For teams: 5–7 crisp “Highlights,” 5–7 “Risks/Watch-outs,” and 3–5 “Plays” (practical moves).
